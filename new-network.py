...
class CrossEntropyCost(object): 
    ...
    def derivative(a, y):
        ...
        num = (a - y)
        #in the next line we add a REALLY small value to make 
        #sure we never divide by zero
        den = (a * (1 - a)) + 0.00000000001 
        ans = num / den
        return ans   #%%%     
        ###
...
class LogLikelihood(object):
    ...
    def fn(a, y):
        """Return the cost associated with an output ``a`` and desired output
        ``y``.  Note that np.nan_to_num is used to ensure numerical
        stability. The np.nan_to_num ensures that null values are converted
        to the correct value (0.0).
        RETURNS A SCALAR
        """
        y_idx = np.argmax(y)
        aly = a[y_idx]
        log_a = np.log(aly)
        #make sure there are no nearly -inf values:
    ...
    def derivative(a, y):
        ...
        final_result = np.zeros((a.shape))
        final_result[y_idx] = res
        #make sure there are no nearly inf values:
        final_result[final_result == np.inf] = 0
        
        return final_result #%%%

        # return np.nan_to_num(1/a[y_idx])        
...
class ReLU(object):
    ...
    def fn(z):
        ...
        ans = np.maximum(vec0, z)
        return ans #%%%

    @classmethod
    def derivative(cls,z):
        """Derivative of the ReLU function."""
        relu_prime = np.array([1 if i > 0 else 0 for i in z])
        return np.reshape(relu_prime, (len(z), 1))  #%%%
...
class Softmax(object):
    ...
    def fn(z):
        ...
        ###
        ### YOU WRITE YOUR CODE HERE
        num = np.nan_to_num(np.exp(z))
        den = sum(np.nan_to_num(np.exp(z)))
        final_answer = np.nan_to_num(num / den)
        return  final_answer #%%%        
        ###
...
class Network(object):
    ...
    def __init__(self, sizes, cost=CrossEntropyCost, act_hidden=Sigmoid, \
        ...
        if self.act_output == Tanh and self.cost != QuadraticCost:
            # override act_output to Sigmoid:
            self.act_output = Sigmoid #%%%
            print('`act_output` set to `Sigmoid`\n')        
        self.regularization = regularization
        self.dropoutpercent = dropoutpercent 
        self.p = 1 - self.dropoutpercent #%%%
        self.masks = None #%%%
        ...
        for j in range(epochs):#xrange(epochs):
            ...
            for mini_batch in mini_batches:
                if self.dropoutpercent > 0.0:
                    # Create 1 set of dropout mask vectors (each hidden layer gets a mask vector)
                    # that will be used for this mini-batch (every record in this batch will go 
                    # through the same network). Every mini-batch gets a different mask and thus
                    # goes through a different network. Note also that we scale the mask by 
                    # dividing by ``p``` as a way of scaling the final result.                 
                    self.masks = [np.random.binomial(1, self.p, n_nodes)/self.p for n_nodes in self.sizes[1:-1]] #%%%               
    ...
    def backprop(self, x, y): 
        ...
        ###################
        # feedforward (forward propogate)
        ###################
        ######
        # Compute activation for the INPUT layer: 
        ######
        activation = x # first "activation" vector is just the input
        activations = [x] # list to store all the activations, layer by layer
        zs = [] # list to store all the z vectors, layer by layer
        
        ######
        # Compute activation for all HIDDEN layers: 
        ######
        if self.masks: #%%%
            for b, w, m in zip(self.biases[:-1], self.weights[:-1], self.masks):
                ...
                dropout_activation = activation * m.reshape((len(m),1)) # multiply by the mask
        ...
        ######
        # Compute gradient for the HIDDEN layers
        ######
    ...
    def total_cost(self, data, lmbda, convert=False):
        ...
        # if we did regularization, add extra cost for large weights:
        if self.regularization == 'L1':  #%%% 
